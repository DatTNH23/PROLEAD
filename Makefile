...
TEST_SRC = "ut src/Hardware src/Software src/Util"
DEBUG_SRC = src
RELEASE_SRC = src
...
HELP_MESSAGE = Simply use any combination of 'make {check, debug, release, help, clean}'. Just calling 'make' will build release and debug. By adding 'V=1' prints more verbose output.
...
directories: check
ifeq ($(D), 2)
	@echo  '_____Building Unit Tests_____'
else ifeq ($(D), 1)
